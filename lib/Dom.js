...
function Dom(model) {
  ...
  function onDomTrigger(name, listener, id, domE, el, next) {
    var delay = listener.delay
      , finish = listener.fn;

    var e = Object.create(domE, {
...
function domClear() {
  ...
  for (var i = this._listeners.length; i--;) {
    removeListener.apply(null, this._listeners[i]);
  }
  this._listeners = [];
  var componentsData = this.model.get('$components');
  for (var key in componentsData) {
    var component = componentsData[key] && componentsData[key].$component;
...
function cleanupComponents() {
  ...
  for (var key in componentsData) {
    var scope = '$components.' + key;
    if (getMarker(map, scope)) continue;
    var component = componentsData[key] && componentsData[key].$component;
    if (component) {
      component.destroy();
    } else {
      silentModel.del(scope);
    }
  }
}
...
function addComponent(ctx, component) {
  var dom = component.dom = Object.create(this);
